...
const configData = fs.readFileSync(
  `${process.cwd()}/prisma/data/credebl-master-table.json`,
  'utf8'
);
...
const createOrgAgentTypes = async (): Promise<void> => {
  try {
    ...
    const orgAgentType = orgAgentTypeData.map(
      (orgAgentType) => orgAgentType.agent
    );
...
const createLedgerConfig = async (): Promise<void> => {
  try {
    ...
    const checkDataIsEqual = (ledgerConfig, ledgerConfigList): boolean => {
      ...
      for (let i = 0; i < ledgerConfig.length; i++) {
        ...
        const config2 = ledgerConfigList.find(
          (item) => item.name === config1.name &&
            JSON.stringify(item.details) === JSON.stringify(config1.details)
        );
    ...
    if (0 === ledgerConfigList.length) {
      ...
      logger.log(
        'Existing ledger config deleted and new ones created:',
        configDetails
      );
    } else {
...
const migrateOrgAgentDids = async (): Promise<void> => {
  try {
    const org_agents = await prisma.org_agents.findMany({
      where: {
        walletName: {
          not: 'platform-admin'
        }
    ...
    const orgDids = org_agents
      .map((agent) => agent.orgDid)
      .filter((did) => null !== did && '' !== did);
    ...
    const filteredOrgAgents = org_agents.filter(
...
const importGeoLocationMasterData = async (): Promise<void> => {
  try {
    ...
    if (!scriptPath || !dbUrl) {
      throw new Error(
        'Environment variables GEO_LOCATION_MASTER_DATA_IMPORT_SCRIPT or DATABASE_URL are not set.'
      );
    }
    ...
    logger.error(
      'An error occurred during importGeoLocationMasterData:',
      error
    );
    throw error;
  }
...
const encryptClientCredential = async (
...
const updateClientCredential = async (): Promise<void> => {
  try {
    ...
    const encryptedClientId = await encryptClientCredential(
      process.env.KEYCLOAK_MANAGEMENT_CLIENT_ID
    );
    const encryptedClientSecret = await encryptClientCredential(
      process.env.KEYCLOAK_MANAGEMENT_CLIENT_SECRET
    );
