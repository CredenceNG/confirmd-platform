  ...
  async walletProvision(payload: IWalletProvision): Promise<object> {
    try {
      ...
      if (agentType === AgentType.AFJ) {
        ...
        } ${process.env.AGENT_HOST} ${process.env.AWS_ACCOUNT_ID} ${
          process.env.S3_BUCKET_ARN
        } ${process.env.CLUSTER_NAME} ${process.env.TESKDEFINITION_FAMILY}`;
        const spinUpResponse: object = new Promise(async (resolve) => {
          await exec(walletProvision, async (err, stdout, stderr) => {
            ...
            const agentTokenPath = `${process.cwd()}${
              process.env.AFJ_AGENT_TOKEN_PATH
            }${orgId}_${containerName}.json`;

            const agentEndPointExists = await this.checkFileExistence(
              agentEndpointPath
            );
            const agentTokenExists = await this.checkFileExistence(
              agentTokenPath
            );
            ...
            if (agentEndPointExists && agentTokenExists) {
              ...
              throw new NotFoundException(
                `${agentEndpointPath} or ${agentTokenPath} files do not exist `
              );
            }
      ...
      this.logger.error(
        `[walletProvision] - error in wallet provision: ${JSON.stringify(
          error
        )}`
      );
