...
export class UserController {
  ...
  @ApiOperation({
    ...
    description:
      "Retrieve publicly available details of a user using their username.",
  })
  @ApiParam({
    name: "username",
    type: String,
  ...
  async getUserActivities(
    ...
    const userDetails = await this.userService.getUserActivities(
      reqUser.id,
      limit
    );
  ...
  @ApiOperation({
    ...
    description:
      "Retrieve a list of invitations received to the user to join organizations.",
  })
  @UseGuards(AuthGuard("jwt"), UserAccessGuard)
  @ApiBearerAuth()
  @ApiQuery({
  ...
  async invitations(
    ...
    if (!Object.values(Invitation).includes(getAllInvitationsDto.status)) {
      throw new BadRequestException(
        ResponseMessages.user.error.invalidInvitationStatus
      );
    }
  ...
  @ApiOperation({
    ...
    description:
      "Check if a user is already registered and if their email already exists.",
  })
  async checkUserExist(
    @Param() emailParam: EmailValidator,
    @Res() res: Response
  ): Promise<Response> {
  ...
  async acceptRejectInvitaion(
    ...
    const invitationRes = await this.userService.acceptRejectInvitaion(
  ...
  @ApiOperation({
    ...
    description:
      "Modify the user profile details such as name, email, or other information.",
  })
  @ApiResponse({
    status: HttpStatus.OK,
    description: "Success",
  ...
  async addPasskey(
    ...
    const userDetails = await this.userService.addPasskey(
      reqUser.email,
      userInfo
    );
    const finalResponse = {
  ...
  @ApiOperation({
    ...
    description:
      "Modify platform settings. Only accessible by platform admins.",
  })
  @UseGuards(AuthGuard("jwt"), OrgRolesGuard, UserAccessGuard)
  @Roles(OrgRoles.PLATFORM_ADMIN)
  @ApiBearerAuth()
  async updatePlatformSettings(
    ...
    const result = await this.userService.updatePlatformSettings(
      platformSettings
    );
  ...
  /**
   * TEST ENDPOINT: Test platform admin authentication
   * This endpoint is for testing the platform admin decryption fix
   */
  @Post("/test-platform-admin-auth")
  @ApiExcludeEndpoint()
  async testPlatformAdminAuth(
    @Body() payload: { userId: string },
    @Res() res: Response
  ): Promise<Response> {
    const result = await this.userService.testPlatformAdminAuth(payload.userId);

    const finalResponse: IResponse = {
      statusCode: HttpStatus.OK,
      message: "Platform admin authentication test completed",
      data: result,
    };
  ...
  async updateUserById(
    ...
    const result = await this.userService.updateUserById(id, userData);
  ...
  async getUserStats(@Res() res: Response): Promise<Response> {
  ...
  /**
   * TEST ENDPOINT: Get profile without authentication
   */
  @Get("/test-profile/:userId")
  @ApiExcludeEndpoint()
  async testGetProfile(
    ...
    const finalResponse: IResponse = {
      statusCode: HttpStatus.OK,
      message: "Test profile fetched successfully",
      data: userData,
    };
