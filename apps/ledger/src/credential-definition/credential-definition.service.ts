...
export class CredentialDefinitionService extends BaseService {
  ...
  async createCredentialDefinition(payload: CreateCredDefPayload): Promise<credential_definition> {
    try {
      ...
      const orgAgentType = await this.credentialDefinitionRepository.getOrgAgentType(
  ...
  async _createCredentialDefinition(payload: CreateCredDefAgentRedirection): Promise<{
    ...
    try {
      ...
      const credDefResponse = await from(this.natsClient.send<string>(this.credDefServiceProxy, pattern, payload))
        .pipe(
          map((response) => ({
            response
          }))
  ...
  async getAllCredDefs(payload: GetAllCredDefsPayload): Promise<ICredDefDetails> {
    try {
      ...
      const filteredResponse = response.filter((credDef) => !archivedSchemaIds.includes(credDef.schemaLedgerId));
  ...
  async getAllCredentialTemplates(orgId: string, schemaType: string): Promise<CredDefSchema[]> {
    ...
    try {
      ...
      if (schemaType === SchemaType.W3C_Schema) {
        const schemaDetailList = await this.credentialDefinitionRepository.getAllSchemaByOrgIdAndType(
          orgId,
          schemaType
        );
        const schemaResponse = await Promise.all(
          schemaDetailList.map(async (schemaDetails) => ({
      ...
      const credDefSchemaList = await this.credentialDefinitionRepository.getAllCredDefsByOrgIdForBulk(payload);
      ...
      this.logger.error(`get Cred-Defs and schema List By OrgId for bulk operations: ${JSON.stringify(error)}`);
