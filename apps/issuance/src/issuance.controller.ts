...
export class IssuanceController {
  ...
  async getIssuanceRecordsByOrgId(payload: {
    orgId: string;
    userId: string;
  }): Promise<number> {
    const { orgId } = payload;
  ...
  async getIssueCredentialsbyCredentialRecordId(
    payload: IIssueCredentialsDefinitions
  ): Promise<string> {
    const { user, credentialRecordId, orgId } = payload;
    return this.issuanceService.getIssueCredentialsbyCredentialRecordId(
      user,
      credentialRecordId,
      orgId
    );
  ...
  async getIssueCredentialWebhook(
    payload: IssueCredentialWebhookPayload
  ): Promise<object> {
    return this.issuanceService.getIssueCredentialWebhook(payload);
  }

  @MessagePattern({ cmd: 'webhook-credential-received' })
  async processCredentialWebhook(@Payload() webhookData: any): Promise<void> {
    this.logger.log('🎓 Processing credential webhook event');
    return this.issuanceService.processCredentialWebhookEvent(webhookData);
  }

  @MessagePattern({ cmd: 'out-of-band-credential-offer' })
  async outOfBandCredentialOffer(
    payload: OutOfBandCredentialOffer
  ): Promise<boolean> {
    const { outOfBandCredentialDto } = payload;
    return this.issuanceService.outOfBandCredentialOffer(
  ...
  async downloadBulkIssuanceCSVTemplate(payload: {
    ...
    return this.issuanceService.downloadBulkIssuanceCSVTemplate(
      orgId,
      templateDetails
    );
  }
  ...
  async uploadCSVTemplate(payload: {
    ...
    return this.issuanceService.uploadCSVTemplate(
      payload.importFileDetails,
      payload.orgId
    );
  }

  @MessagePattern({ cmd: 'preview-csv-details' })
  async previewCSVDetails(payload: {
    requestId: string;
    previewFileDetails: PreviewRequest;
  }): Promise<object> {
  ...
  async issuedFiles(payload: {
    orgId: string;
    fileParameter: PreviewRequest;
  }): Promise<object> {
  ...
  async getFileDetailsByFileId(payload: {
    fileId: string;
    fileParameter: PreviewRequest;
  }): Promise<object> {
  ...
  async deleteIssuanceRecords(payload: {
    orgId: string;
    userDetails: user;
  ...
  async getFileDetailsAndFileDataByFileId(payload: {
    fileId: string;
    orgId: string;
  }): Promise<object> {
